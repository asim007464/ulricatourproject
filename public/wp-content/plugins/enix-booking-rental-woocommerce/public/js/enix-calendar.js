...
jQuery(document).ready(function ($) {
  ...
  $(".enix-calendar-widget").each(function () {
    ...
    var unavailableDates = []; // Admin-blocked dates from API
    ...
    // Load admin-blocked dates (multiple bookings per day are still allowed)
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            ...
            } else if (response.data.booked_dates) {
              unavailableDates = response.data.booked_dates;
            }
          }
    ...
    function isUnavailableDate(formatted, dateObj, dateToday) {
      if (unavailableDates.includes(formatted)) {
        return true;
      }
      return dateObj.getTime() < dateToday.getTime();
    }
    ...
    function renderMonthView() {
      ...
      cells.forEach(function (cell, index) {
        ...
        if (cell.otherMonth) {
          ...
          var isBlocked = isUnavailableDate(formatted, cell.date, dateToday);
    ...
    function renderWeekView() {
      ...
      var titleText =
        monthNames[monday.getMonth()].substring(0, 3) +
        " " +
        monday.getDate() +
        " – ";
      ...
      for (var i = 0; i < 7; i++) {
        ...
        // Header: "Mon 1"
        $trHead.append(
          "<th>" + dayNames[i] + " " + dayDate.getDate() + "</th>",
        );
        ...
        $td.attr("data-date", formatted);
        $td.append(
          '<span class="enix-cal-day-num">' + dayDate.getDate() + "</span>",
        );
        $td.append(
          '<span class="enix-cal-day-price">' +
            currencySym +
        ...
        var isBlocked = isUnavailableDate(formatted, dayDate, dateToday);
    ...
    function renderDayView() {
      ...
      $title.text(
        dayOfWeek + ", " + monthNames[month] + " " + dateVal + ", " + year,
      );
      ...
      var isBlocked = isUnavailableDate(formatted, currentDate, dateToday);
      ...
      if (isBlocked) {
        ...
        statusLabel = isPast ? "Unavailable (Past Date)" : "Unavailable";
      }
      ...
      var $dayCard = $(
        '<div class="enix-cal-list-item" style="border: 1px solid var(--enix-border); border-radius: 8px; padding: 20px;">' +
          ...
          '<p style="margin: 0; color: var(--enix-gray-text);">Base Price: <strong>' +
          currencySym +
          basePrice +
          "</strong></p>" +
          "</div>" +
          '<span class="enix-cal-list-status ' +
          statusClass +
          '">' +
          statusLabel +
          "</span>" +
    ...
    function renderListView() {
      ...
      for (var i = 0; i < 15; i++) {
        ...
        var isBlocked =
          ...
          listDate.getTime() < dateToday.getTime();
        ...
        if (isBlocked) {
          statusClass = "unavailable";
          statusLabel = unavailableDates.includes(formatted)
            ? "Unavailable"
            : "Past";
        }
        ...
        var displayDate =
          dayName +
          ", " +
          monthNames[listDate.getMonth()].substring(0, 3) +
          " " +
          listDate.getDate();

        var $item = $(
          '<li class="enix-cal-list-item">' +
            '<span class="enix-cal-list-date">' +
            displayDate +
            " - " +
            currencySym +
            basePrice +
    ...
    $calendarBox.on("click", ".enix-cal-cell.available", function () {
      ...
      var $form = $(
        '.enix-booking-form-widget .enix-bf-form[data-product-id="' +
          productId +
          '"]',
      );
      if (!$form.length) {
      ...
      if (!$hint.length) {
        $hint = $(
          '<div class="enix-cal-hint" style="display:none;margin:10px 0;padding:10px 14px;border-radius:6px;background:#fff5ef;color:#c75010;border:1px solid var(--enix-orange,#f26522);font-weight:600;text-align:center;"></div>',
        );
      ...
      if (!pickupVal) {
        ...
        if ($dropoff.length) {
          showHint(
            "Pick-up Date selected. Now please select a Drop-off Date.",
            "info",
          );
        }
        ...
        if (clickedDate < pickupDate) {
          setFlatpickrValue($pickup, formatted);
          showHint(
            "Pick-up Date updated. Now please select a Drop-off Date.",
            "info",
          );
        } else {
          setFlatpickrValue($dropoff, formatted);
          showHint(
            "Drop-off Date selected. You can now submit your booking.",
            "success",
          );
        }
      } else {
