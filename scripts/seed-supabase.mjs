...
function extractProductCoverImageUrl(html) {
  for (const elementId of ["elementor-element-b48889c", "elementor-element-90dc87b"]) {
...
function extractListingCardImages(pageHtml) {
...
async function seedCategory(manifestPath, category, listingImages) {
  ...
  for (const product of manifest.products) {
    ...
    const coverImageUrl =
      extractProductCoverImageUrl(html) || extractProductImageUrl(html);
    const listingImage = parsed.wordpress_id
      ? listingImages.get(parsed.wordpress_id)
      : null;
...
const tourListingImages = extractListingCardImages(
  fs.readFileSync(
    path.join(root, "src/content/tours-page-body.html"),
    "utf8"
  )
);
const taxiListingImages = extractListingCardImages(
  fs.readFileSync(
    path.join(root, "src/content/transportation-page-body.html"),
    "utf8"
  )
);
